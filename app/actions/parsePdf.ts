...
【重要な抽出ルール】
1. schedulesには全ての研究科・専攻・コースを含める。省略厳禁。
2. 【専攻・コースの分割ルール】同一の研究科に複数の専攻・コース・分野・プログラム・専修がある場合は、それぞれを必ず別のエントリとして作成する。
   - 例：「経済学研究科」に「経済学専攻」と「経営学専攻」がある → schedulesに2つのエントリを作成
   - 例：「工学研究科」に「情報コース」と「機械コース」がある → schedulesに2つのエントリを作成
   - 専攻・コースを表す表現：専攻、コース、分野、プログラム、専修、系、領域
3. 専攻・コースの記載が一切ない場合のみspecializationをnullにする。
4. sessionsには全ての試験回（第1次・第2次、夏季・冬季、前期・後期など）を全て含める。
5. 試験回が1回のみの場合もsessions配列に1要素として必ず含める。
6. 日付の抽出について：
...
7. 日付は必ずYYYY-MM-DD形式に変換する。
   - 令和1年=2019年、令和2年=2020年、令和3年=2021年、令和4年=2022年
   - 令和5年=2023年、令和6年=2024年、令和7年=2025年、令和8年=2026年
8. 情報が見当たらない場合のみnullを返す。テキストに記載がある日付は必ず抽出すること。
9. university_typeが判断できない場合はnullを返す。
10. 必ずJSONのみを返し、説明文・コードブロック記号は不要。`
